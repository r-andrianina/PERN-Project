...
5. **`docker-compose.yml` has a hardcoded local Postgres password** (`1346h`) committed in plain text — low risk since it's dev-only and port 5435 is not typically exposed, but worth confirming it's never reused in `.env.production`.
