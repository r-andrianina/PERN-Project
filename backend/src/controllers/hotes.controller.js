...
const listHotes = async (req, res) => {
  const hotes = await service.list(req.query, req.user);
  res.json({ total: hotes.length, hotes });
};

const getOne = async (req, res) => res.json({ hote: await service.getById(parseInt(req.params.id), req.user) });
...
const update = async (req, res) => {
  ...
  const hote   = await service.update(id, req.body, req.user);
