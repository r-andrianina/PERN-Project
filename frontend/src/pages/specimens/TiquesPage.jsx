  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        title="Tiques" subtitle={`${total} spécimen(s) au total`}
        actions={
          <>
      ...
      {loading && !data ? <Spinner.Block /> : total === 0 && !debounced ? (
        ...
        <Card padding="none" className="overflow-hidden">
          <div className="px-4 py-3 border-b border-border flex items-center gap-3">
            <div className="flex items-center gap-2.5 flex-1 border border-border-strong rounded-xl px-3.5 py-2 bg-surface-2 focus-within:bg-surface focus-within:border-primary transition-all">
              ...
              <input type="text" placeholder="Rechercher par espèce, ID terrain ou notes…"
            ...
            <span className="text-xs text-fg-subtle whitespace-nowrap font-medium">{total} résultat(s)</span>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[900px]">
              ...
              <tbody className="divide-y divide-border">
                {loading ? (
                  <tr><td colSpan={11} className="text-center py-8 text-fg-subtle text-sm">Chargement…</td></tr>
                ) : tiques.length === 0 ? (
                  <tr><td colSpan={11} className="text-center py-8 text-fg-subtle text-sm">Aucun résultat pour « {debounced} »</td></tr>
                ) : tiques.map(t => (
                  <tr key={t.id} className="hover:bg-specimen-tique/5 transition-colors cursor-pointer"
                    ...
                    <td className="px-4 py-3">
                      {t.idTerrain ? <Badge tone="primary" size="sm" className="font-mono font-bold">{t.idTerrain}</Badge>
